...
- 以降は順にぐるぐる回り、同じ形・同じ枚数で **同じ色の上の数字** か **同じ数字**（色は問わない。同じ色の2枚目でもよい）を重ねるか、パスして抜ける。**連番と同数は色を問わず、数字が上なら重ねられる**。いちばん大きい数字（4人なら13）の上には1を出せる（1周。1枚は同じスートの1〔ゴリラはどの色でも〕、同数は色を問わず、連番は 12・13 の上に 1・2 のように1から始まる連番。バナナ・2 も 1・2 になる）。同数は同じ数字の同数も重ねられる
